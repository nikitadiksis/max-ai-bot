...
SERVICE_MONITOR_ENABLED=1
SERVICE_MONITOR_INTERVAL_MINUTES=15
ALERT_HIGH_ERRORS_WINDOW_MINUTES=30
ALERT_HIGH_ERRORS_THRESHOLD=8
ALERT_LOW_PAYMENTS_LOOKBACK_HOURS=24
ALERT_LOW_PAYMENTS_MIN_ACTIVE_USERS=15
ALERT_LOW_PAYMENTS_MAX_PAYMENTS=0
ALERT_SPEND_SPIKE_LOOKBACK_HOURS=1
ALERT_SPEND_SPIKE_MIN_RUB=150
ALERT_SPEND_SPIKE_MULTIPLIER=3.0
...
When ads send people to the channel first, keep using campaign promo codes for attribution.
The bot also checks channel membership before normal usage when `CHANNEL_GATE_ENABLED=1`:
...
- `CHANNEL_CHAT_ID` can be left empty if `CHANNEL_URL` contains the MAX channel id/slug; set it manually if MAX requires another id
...
Рефералка:
- код можно применить вручную через `/ref CODE`
- кнопка `Поделиться` открывает MAX-шторку с готовым приглашением и ссылкой на канал
